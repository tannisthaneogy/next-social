    ...
    return (
        <div className="p-4 bg-[#2a2a30] rounded-lg shadow-md text-sm flex flex-col gap-6">
            <div className="h-20 relative">
                ...
                <Image src={user.avatar || "/noAvatar.png"} alt="" width={48} height={48} className="rounded-full object-cover w-12 h-12 absolute left-0 right-0 m-auto -bottom-6 ring-1 ring-[#FEFD03] z-10"/>
            </div><div className="h-24 flex flex-col gap-2 items-center text-gray-400">
                <span className="font-semibold mt-2">{(user.name && user.surname) ? user.name + " " + user.surname : user.username}</span>
